...
def detect_backhands(video_path, output_dir, log_callback=print):
    ...
    keras_model = tf.keras.models.load_model(
        os.path.join(BASE_DIR, "models/tennis_stroke/tennis_model_keras")
    )
    rejector = tf.keras.models.load_model(
        os.path.join(BASE_DIR, "models/tennis_stroke/skeleton_rejector")
    )
    rejector.trainable = False

    le = joblib.load(
        os.path.join(BASE_DIR, "models/tennis_stroke/label_encoder_keras.pkl")
    )

    log("Initializing MediaPipe")

    base_options = python.BaseOptions(
        model_asset_path=os.path.join(BASE_DIR, "pose_landmarker_lite.task")
    )
